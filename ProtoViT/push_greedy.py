...
def save_prototype_original_img_with_bbox(dir_for_saving_prototypes, 
    ...
    """
    save_dir = os.path.join(dir_for_saving_prototypes, f'{str(j)}_subprototype{filename_sufix_prototype_img}.png')
    save_dir2 = os.path.join(dir_for_saving_prototypes, f'{str(j)}_prototype{filename_sufix_prototype_img}.png')
    ...
    #save_dir3 = os.path.join(dir_for_saving_prototypes,
                 #filename_sufix_prototype_img + '_vis_bb_' + str(j) +'.png')
...
def update_prototypes_on_batch(search_batch_input,
                               ...
                               filename_sufix_prototype_img=None,
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            if dir_for_saving_prototypes is not None:
                original_img_path = os.path.join(
                    ...
                    f'{str(j)}_image{filename_sufix_prototype_img}.png'
                )
                plt.imsave(
                    original_img_path,
                    original_img_j,
                ...
                #                 filename_sufix_prototype_img + 'bbox-original' + str(j) +'.png')
                save_prototype_original_img_with_bbox(dir_for_saving_prototypes, 
                                                    original_img_path,
                                                    filename_sufix_prototype_img,
                                                    j = j,
                                                    sub_patches = n_p,
                                                    indices = min_j_indice,
                                                    bound_box_j = proto_bound_boxes[j], 
                                                    color = (0, 255, 255))
                # rt_newvis = os.path.join(dir_for_saving_prototypes,
                #                 filename_sufix_prototype_img + '_newvis_' + str(j) +'.png')
...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
                    ...
                    filename_sufix_prototype_img=None,
                    # prototype_self_act_filename_prefix=None,
                    filename_prefix_prototype_bb=None,
    ...
    if proto_epoch_dir != None and filename_prefix_prototype_bb != None:
        if epoch_number != None:
            np.save(os.path.join(proto_epoch_dir, filename_prefix_prototype_bb + str(epoch_number) + '.npy'),
                    proto_bound_boxes)
        else:
            np.save(os.path.join(proto_epoch_dir, filename_prefix_prototype_bb + '.npy'),
                    proto_bound_boxes)
