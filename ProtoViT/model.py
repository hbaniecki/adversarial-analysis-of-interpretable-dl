...
class PPNet(nn.Module):
    def __init__(
            self, features, img_size, prototype_shape, num_classes, 
            init_weights=True,
        ...
        #print(features_name)
        if features_name.startswith('DEIT') or features_name.startswith('VISION'):
            self.arc = 'deit'
    ...
    def neigboring_mask(self, center_indices):
        ...
        start_row = (center_row+self.radius - small_size // 2).clone().detach()
        start_col = (center_col+self.radius - small_size // 2).clone().detach()
