...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for j in range(n_prototypes):
        ...
        for i, patch in enumerate(heaps[j]):
            ...
            plt.imsave(fname=os.path.join(root_dir_for_saving_images, f'{str(j)}_{str(i)}_nearest_image.png'),
                           arr=patch.original_img,
                           vmin=0.0,
                           vmax=1.0)
            bb_dir = os.path.join(root_dir_for_saving_images, f'{str(j)}_{str(i)}_nearest_patch.png')
        ...
        np.save(os.path.join(root_dir_for_saving_images, 'class_id.npy'), labels)
