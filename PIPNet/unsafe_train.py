...
from unsafe_utils import read_args_from_file
import argparse
...
def run_pipnet(args=None):
    ...
    if torch.cuda.is_available():
        ...
        raise ValueError("This script requires a GPU")
     
    # Log which device was actually used
    print("Device used: ", device, flush=True)
    
    # Obtain the dataset and dataloaders
    trainloader, trainloader_pretraining, _, _, projectloader, testloader, testloader_bs1, classes = get_dataloaders(args)
    if len(classes) <= 20:
    ...
    optimizer_net, optimizer_classifier, params_to_train, params_backbone = get_optimizer_nn(net, args)   
    ...
    with torch.no_grad():
        if args.state_dict_dir_net != '':
            ...
            try:
                ...
                pass
            if torch.mean(net._classification.weight).item() > 1.0 and torch.mean(net._classification.weight).item() < 3.0 and torch.count_nonzero(torch.relu(net._classification.weight-1e-5)).float().item() > 0.8*(num_prototypes*len(classes)): #assume that the linear classification layer is not yet trained (e.g. when loading a pretrained backbone only)
                ...
                print("Classification layer initialized with mean", torch.mean(net._classification.weight).item(), flush=True)
                if args.bias:
    ...
    if net._num_classes == 2:
        ...
        log.create_log('log_epoch_overview', 'epoch', 'test_top1_acc', 'test_f1', 'almost_sim_nonzeros', 'local_size_all_classes',
                       'almost_nonzeros_pooled', 'num_nonzero_prototypes', 'mean_train_acc', 'mean_train_loss_during_epoch')
        ...
        log.create_log('log_epoch_overview', 'epoch', 'test_top1_acc', 'test_top5_acc', 'almost_sim_nonzeros', 'local_size_all_classes',
                       'almost_nonzeros_pooled', 'num_nonzero_prototypes', 'mean_train_acc', 'mean_train_loss_during_epoch')
    ...
    for epoch in range(1, args.epochs_pretrain+1):
        ...
        for param in net._classification.parameters():
    ...
    with torch.no_grad():
        if args.seed == 0 and args.epochs_pretrain > 0:
    ...
    for epoch in range(1, args.epochs + 1):                      
        if epoch <= args.epochs_finetune and (args.epochs_pretrain > 0 or args.state_dict_dir_net != ''):
            for param in net._add_on.parameters():
                param.requires_grad = False
            for param in params_to_train:
                param.requires_grad = False
        ...
        if (epoch == args.epochs or epoch % 10 == 0) and args.epochs > 1:
            # SET SMALL WEIGHTS TO ZERO
            with torch.no_grad():
                torch.set_printoptions(profile="full")
                # can't interfer with the weights of the classification layer during test time
                # net._classification.weight.copy_(torch.clamp(net._classification.weight.data - 0.001, min=0.)) 
                print("Classifier weights: ", 
                      net._classification.weight[torch.clamp(net._classification.weight.data - 0.001, min=0.).nonzero(as_tuple=True)], 
                      (net._classification.weight[net._classification.weight.nonzero(as_tuple=True)]).shape, flush=True)
                if args.bias:
                    print("Classifier bias: ", net._classification.bias, flush=True)
        ...
        for prot in topks.keys():
            ...
            for (i_id, score) in topks[prot]:
                if score > 0.1:
                    found = True
            if not found:
                torch.nn.init.zeros_(net._classification.weight[:, prot])
    ...
    print("Classifier weights: ", net._classification.weight, flush=True)
    print("Classifier weights nonzero: ", net._classification.weight[net._classification.weight.nonzero(as_tuple=True)], (net._classification.weight[net._classification.weight.nonzero(as_tuple=True)]).shape, flush=True)
    print("Classifier bias: ", net._classification.bias, flush=True)
    # Print weights and relevant prototypes per class
    for c in range(net._classification.weight.shape[0]):
        relevant_ps = []
        proto_weights = net._classification.weight[c,:]
        for p in range(net._classification.weight.shape[1]):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size_pretrain', default=None, type=int)
    ...
    args.log_dir = args.log_dir + f'/train_{args.dataset}_{args.net}_{args.seed}'
