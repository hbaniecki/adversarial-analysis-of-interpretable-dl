...
def train_pipnet(net, train_loader, optimizer_net, optimizer_classifier, scheduler_net, scheduler_classifier, 
                 criterion, epoch, nr_epochs, device, pretrain=False, finetune=False, progress_prefix: str = 'Train Epoch'):
    ...
    for i, (xs1, xs2, ys) in train_iter:       
        ...
        loss, acc = calculate_loss(proto_features, pooled, out, ys, align_pf_weight, t_weight, unif_weight, cl_weight, 
                                   net._classification.normalization_multiplier, pretrain, finetune, criterion, 
        ...
        if not pretrain:
            with torch.no_grad():
                ...
                # net._classification.weight.copy_(torch.clamp(net._classification.weight.data - 1e-3, min=0.))
                net._classification.weight.copy_(net._classification.weight.data * (net._classification.weight.data > 1e-3).float())
                net._classification.normalization_multiplier.copy_(torch.clamp(net._classification.normalization_multiplier.data, min=1.0)) 
                if net._classification.bias is not None:
                    net._classification.bias.copy_(torch.clamp(net._classification.bias.data, min=0.))  
