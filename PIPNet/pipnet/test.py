...
def eval_pipnet(net,
    ...
    for i, (xs, ys) in test_iter:
        ...
        with torch.no_grad():
            ...
            ys_pred_scores = torch.amax(F.softmax((torch.log1p(out**net._classification.normalization_multiplier)),dim=1),dim=1)
            ...
            cm_batch = np.zeros((net._num_classes, net._num_classes), dtype=int)
    ...
    info['num non-zero prototypes'] = torch.gt(net._classification.weight,1e-3).any(dim=0).sum().item()
    print("sparsity ratio: ", (torch.numel(net._classification.weight)-torch.count_nonzero(torch.nn.functional.relu(net._classification.weight-1e-3)).item()) / torch.numel(net._classification.weight), flush=True)
