...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--epochs_finetune',
                        type=int,
                        default=5,
                        help='during finetuning, only train classification layer and freeze rest. usually done for a few epochs (at least 1, more depends on size of dataset'
                        )
...
def get_optimizer_nn(net, args: argparse.Namespace) -> torch.optim.Optimizer:
    ...
    params_backbone = []
    # set up optimizer   
    if 'resnet18' in args.net or 'resnet34' in args.net: 
        print("chosen network is resnet18 or resnet32", flush=True)
        if args.mode == 'train':
        ...
        if args.mode == 'train':
            ...
            for name, param in net._net.named_parameters():
                if 'layer1' in name or 'layer2' in name:
                    ...
                    param.requires_grad = False
    elif 'resnet101' in args.net or 'resnet152' in args.net: 
        print("chosen network is resnet101 or resnet152", flush=True)
        if args.mode == 'train':
            for name, param in net._net.named_parameters():
        ...
        for name, param in net._net.named_parameters():
            if 'features.7' in name or 'features.6' in name:
                params_to_train.append(param)
            else:
                params_backbone.append(param)
    else:
    ...
    paramlist_net = [
            ...
            {"params": net._add_on.parameters(), "lr": args.lr_block*10., "weight_decay_rate": args.weight_decay}
    ]
    ...
    if args.optimizer == 'Adam':
        ...
        return optimizer_net, optimizer_classifier, params_to_train, params_backbone
    else:
