...
def vis_pred(net, vis_test_dir, classes, device, args, img_suffix=''):
    print("Visualizing a few predictions...", flush=True)
    ...
    if isinstance(vis_test_dir, str):
    ...
    elif isinstance(vis_test_dir, torch.utils.data.DataLoader):
        vis_test_loader = vis_test_dir
        vis_test_set = vis_test_loader.dataset
    ...
    for k, (xs, ys) in enumerate(vis_test_loader): #shuffle is false so should lead to same order as in imgs
        if ys[0] != last_y:
            ...
            count_per_y +=1
            if count_per_y > 10: # show max 10 imgs per class to speed up the process
                continue
        xs, ys = xs.to(device), ys.to(device)

        path_img = imgs[k][0]
        name_img = os.path.splitext(os.path.basename(path_img))[0]
        path_cp = os.path.join(save_dir, name_img + img_suffix)
        if not os.path.exists(path_cp):
            os.makedirs(path_cp)
        shutil.copy(path_img, path_cp)

        path_input = os.path.join(path_cp, f'input{img_suffix}.png')
        plt.imsave(path_input, inverse_normalize(xs[0]).permute(1, 2, 0).detach().cpu().numpy())
        ...
        with torch.no_grad():
            ...
            for pred_class_idx in sorted_out_indices[:3]:
                ...
                save_path = os.path.join(path_cp, pred_class+"_"+str(f"{out[0,pred_class_idx].item():.3f}"))
                ...
                for prototype_idx in sorted_pooled_indices:
                    simweight = pooled[0,prototype_idx].item() * net._classification.weight[pred_class_idx, prototype_idx].item()
                    simweights.append(simweight)

                for i, prototype_idx in enumerate(sorted_pooled_indices):
                    simweight = simweights[i]
                    if abs(simweight) / (np.sum(simweights) + 1e-9) > 0.05 and pooled[0, prototype_idx].item() > 0.33:
                        ...
                        img_patch.save(os.path.join(save_path, 'mul%s_p%s_sim%s_w%s_patch.png'%(str(f"{simweight:.3f}"),str(prototype_idx.item()),str(f"{pooled[0,prototype_idx].item():.3f}"),str(f"{net._classification.weight[pred_class_idx, prototype_idx].item():.3f}"))))
                        draw = D.Draw(image)
                        ...
                        image.save(os.path.join(save_path, 'mul%s_p%s_sim%s_w%s_rect.png'%(str(f"{simweight:.3f}"),str(prototype_idx.item()),str(f"{pooled[0,prototype_idx].item():.3f}"),str(f"{net._classification.weight[pred_class_idx, prototype_idx].item():.3f}"))))
...
def vis_pred_experiments(net, imgs_dir, classes, device, args: argparse.Namespace):
    print("Visualizing the chosen predictions...", flush=True)
    # Make sure the model is in evaluation mode
    net.eval()
