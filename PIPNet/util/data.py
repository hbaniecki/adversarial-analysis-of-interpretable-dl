...
def get_data(args: argparse.Namespace): 
    ...
    if args.dataset =='CUB-200-2011':     
        return get_birds(
            True, 
            train_dir=os.path.join(PATH_DATASETS, 'cub200/dataset/train'),
            project_dir=os.path.join(PATH_DATASETS, 'cub200/dataset/train'), 
            test_dir=os.path.join(PATH_DATASETS, 'cub200/dataset/test'), 
            img_size=args.image_size, seed=args.seed, validation_size=args.validation_size, 
            train_dir_pretrain=os.path.join(PATH_DATASETS, 'cub200/dataset/train'),
            test_dir_projection=os.path.join(PATH_DATASETS, 'cub200/dataset/test')
        )
    ...
    elif args.dataset == 'partimagenet': #use --validation_size of 0.2
    ...
    elif args.dataset == 'grayscale_example':
        return get_grayscale(True, './data/train', './data/train', './data/test', args.image_size, args.seed, args.validation_size)
    elif args.dataset == "isic2019":
        return get_isic2019(
            args.augment, 
            train_dir=os.path.join(PATH_DATASETS, 'isic2019/train'), 
            project_dir=os.path.join(PATH_DATASETS, 'isic2019/train'), 
            test_dir=os.path.join(PATH_DATASETS, 'isic2019/test'), 
        ...
        return get_mura(
            args.augment, 
            train_dir=os.path.join(PATH_DATASETS, 'MURA-v1.1/train'), 
            project_dir=os.path.join(PATH_DATASETS, 'MURA-v1.1/train'), 
            test_dir=os.path.join(PATH_DATASETS, 'MURA-v1.1/test'), 
            img_size=args.image_size, 
            seed=args.seed, 
            validation_size=args.validation_size
        )
...
def get_dataloaders(args: argparse.Namespace):
    ...
    if hasattr(args, 'batch_size_pretrain'):
        pretrain_batchsize = args.batch_size_pretrain
    else:
        pretrain_batchsize = args.batch_size
    ...
    testloader_bs1 = torch.utils.data.DataLoader(testset, 
                                                 batch_size=1, 
                                                 shuffle=False,
                                                 pin_memory=cuda, 
                                                 drop_last=False,
    ...
    return trainloader, trainloader_pretraining, trainloader_normal, trainloader_normal_augment, projectloader, testloader, testloader_bs1, classes


def create_datasets(transform1, transform2, transform_no_augment, num_channels:int, train_dir:str, 
                    project_dir: str, test_dir:str, seed:int, validation_size:float, 
                    train_dir_pretrain=None, test_dir_projection=None, transform1p=None):
    ...
    trainset_normal_augment = torch.utils.data.Subset(torchvision.datasets.ImageFolder(train_dir, transform=transforms.Compose([transform1, transform2])), 
    ...
    if train_dir_pretrain is not None:
        ...
        trainset_pretraining = torch.utils.data.Subset(TwoAugSupervisedDataset(trainvalset_pr, transform1=transform1p, transform2=transform2), 
...
def get_mura(augment:bool, train_dir:str, project_dir: str, test_dir:str, img_size: int, seed:int, validation_size:float):
...
def get_isic2019(augment:bool, train_dir:str, project_dir: str, test_dir:str, img_size: int, seed:int, validation_size:float): 
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)
    normalize = transforms.Normalize(mean=mean, std=std)
    transform_no_augment = transforms.Compose([
...
def get_birds(augment: bool, train_dir:str, project_dir: str, test_dir:str, img_size: int, seed:int, validation_size:float, 
              train_dir_pretrain = None, test_dir_projection = None): 
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)
    normalize = transforms.Normalize(mean=mean,std=std)
    transform_no_augment = transforms.Compose([
...
class TrivialAugmentWideMura(transforms.TrivialAugmentWide):
    def _augmentation_space(self, num_bins: int) -> Dict[str, Tuple[Tensor, bool]]:
        return {
            "Identity": (torch.tensor(0.0), False),
